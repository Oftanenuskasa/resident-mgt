...
";
	...
	<th width="1047" height="300" valign="top" bgcolor="white"><br>
		...
		$result = mysqli_query($conn, "SELECT *FROM idcard where FirstName='$fname' and middleName='$mname' ");
		...
		if ($con > 0) {
		?>
			<div><b> Welcome to renew or missing Idcard request form</b></div>
			...
			<fieldset>
				...
				<br>
				<div>
					<form name="request" onsubmit='return check()' action='aginrequset.php' method='POST'>

						<table align="center">

							<tr>
								<td>Id number</td>
								<td><input type='text' size='15px' name='first' value='<?php echo $id; ?>' readonly></td>
							</tr>
							<tr>
								<td>Purpose</td>
								<td><select id="in" name="com" class="tf" required="">'
										<option selected></option>

										<option value='renew'>Request for renew id card</option>
										<option value='missing'>Request for messing id card</option>
									</select></td>
							</tr>
			...
			if (isset($_POST['submit'])) {
				...
				$sql = "SELECT *FROM `idcard` WHERE id_number = '$id'";
				...
				if ($da == $renw and $content == 'renew') {
					echo "<font color='white'><h4 align='left' style='width:520px;height:40;background:brown'><font color='grean' size='6'>(!)</font> ID is allredy renwed in Today!!</h4></font";
				} else if ($da >= $renw and $da < $exp and $content == 'renew')
					echo "<font color='white'><h4 align='left' style='width:520px;height:40;background:brown'><font color='grean' size='6'>(!)</font>  In this year you are not allowed to renew!!</h4></font";
				else {
					$sql = "SELECT  * FROM `user` WHERE Username='$_username'";
					$re = mysqli_query($conn, $sql);
					if (!$re)
						echo " qurey Not exexuted ";
					else {
						while ($row = mysqli_fetch_array($re)) {
							$fname = $row['frist_name'];
							$mname = $row['mname'];
					...
					$sql = "INSERT INTO `requestr`(`id`,`sendername`, `SenderidNumber`, `Date`, `Purpose`,`seen`) VALUES
('none','$_username','$_POST[first]','$da', '$_POST[com]','0')";
					$result = mysqli_query($conn, $sql);
					if (!$result) {
						die('Error: ' . mysqli_error($conn));
					} else
						echo "<font color='white'><h4 align='center' style='width:520px;height:40;background:brown'> Your request successfully sent!!</h4></font";
				}
			}
		} else
			echo "<font color='white'><h4 style='width:535px;height:55;background:red' align='left'><font color='grean' size='6'>(!)</font>  You are not allowed to acces this page.To acces this page you must request for prepare Id card first.</font>";
