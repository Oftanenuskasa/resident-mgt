...
";
	...
	<th width="1047" height="300" valign="top" bgcolor="white"><br>
		...
		if (isset($_GET['status'])) {
			$update = "UPDATE `noti` SET `seen`=1 WHERE username='$id'";
			...
			while ($row = mysqli_fetch_object($select_status)) {
				...
				if ($update) {
					$sql = "UPDATE `noti` SET `seen`=1 WHERE username='$status'";
					$re = mysqli_query($conn,$sql);
					echo "<h4 style='width:500px;height:30px;background:brown'><font color='white'>The account is deactivated succesfully.</font></h4>";
				} else {
