...
";
	...
	<th width="1047" height="436" valign="top" bgcolor="white"><br>
		...
		<?php
		$id = $_REQUEST['key1'];
		echo "<font color='red'> <h4 align='left' style=\"width: 540px;height: 45px;background:#32CD32

;\"> This is <font color='blue'>$id </font>requset detail.click id number to renw id card or prepare the missing idcard.</h3></font>";
		$result = mysqli_query($conn,"SELECT * FROM requestr  where sendername='$id' and seen='0' ");


		echo "<table id='vtable' style='width:500px;border:1px solid #eee;' align='center' valign='top'><font color=white>
<tr>
<th bgcolor='#eee'><font color='black' size='2'> From(sender) </th>
<th bgcolor='#eee'><font color='black' size='2'> Id number</th>
<th bgcolor='#eee'><font color='black' size='2'>Purpose          </th>
<th bgcolor='#eee'><font color='black' size='2'>Date of requset         </th>
<th bgcolor='#eee'><font color='black' size='2'>Reject          </th>
</tr>";

		while ($row = mysqli_fetch_array($result)) {
			$num = $row["SenderidNumber"];
			$name = $row['sendername'];
			$ctrl = $row['Purpose'];
			$da = $row['Date'];
			$id = $row['id'];
			...
			echo "</td>";
			if ($ctrl == 'renew') {
				echo ("<td align = 'center' width = '1'><a href = 'renew_id.php?id=" . $num . "& name=" . $name . "' onClick=\"return confirm('Are you shure to accept the request.')\">$num</a>
    </td>");
			} else {
				echo ("<td align = 'center' width = '1'><a href = 'giveidcard.php?id=" . $num . "& name=" . $name . "' onClick=\"return confirm('Are you shure to accept the request.')\">$num</a>
    </td>");
			}
			echo "<td>";
			echo $row["Purpose"];
			...
			echo $row["Date"];
			echo "</td>";
			echo ("<td align = 'center' width = '1'><a href = 'delete_requset_renew.php?content=" . $id . "' onClick=\"return confirm('Are you shure to delete.')\"><img src = 'image/close.png' border = '0' title='Delete';'></img></a>
    </td>");
			echo ("</tr>");
		}
		echo ("</table>");
		...
		if ($up)
			...
			echo "no";
		echo "<a href = 'viewrenew_request.php'><img src = 'image/b.png' width='40'border = '0' title='Back to view Idcard Renew requset page';'></img>";
		mysqli_close($conn);
		?>
		</td>
