		...
		<tr>
			<td>
				<table border="0" bgcolor="#CCCCCC" cellspacing="0">
					<tr>
						<td width="250" height="490" valign="top">
	...
	<th width="1047" height="436" valign="top" bgcolor="white"><br>
				...
				<div style="width:400px;">
					...
					<table border="0" cellpadding="10" cellspacing="0" width="100%" align="center" class="tblSaveForm">
						<tr>
							...
							<td width="60%"><input type="password" required name="oldpassword" id="oldpassword" style="width: 200;height: 30;" placeholder="Enter Old password"></td>
						</tr>
						<tr>
							<td><label>New Password:</label></td>
							<td><input type="password" required name="newpassword" id="newPassword" style="width: 200;height: 30;" placeholder="Enter new password"></td>
						</tr>
						<td><label>Confirm Password:</label></td>
						<td><input type="password" required name="cnewpassword" id="cnewpassword" style="width: 200;height: 30;" placeholder="Enter agin the new Password"></td>
						</tr>
						<tr>
		...
		if (isset($_POST['change'])) {
			...
			if (strlen($_POST['newpassword']) < 6 || strlen($_POST['newpassword']) > 25) {
				...
				echo "<font color='red'>Please enter a confirm new password between 6 and 25 characters.</font>";
			} elseif ($_POST['newpassword'] != $_POST['cnewpassword']) {
				echo "<font color='red'>Confirm new password and new password must be equal.</font>";
			} else {
				...
				$username = $_POST['username']; // Assuming username is also posted in the form

				// Prepare and execute the select statement
				$stmt = $conn->prepare("SELECT * FROM user WHERE password=? AND Username=?");
				$stmt->bind_param("ss", $oldpassword, $username);
				$stmt->execute();
				...
				if ($result->num_rows > 0) {
					...
					if ($stmt->execute()) {
						echo "<h4 style='width:530px;height:30px;background:red'><font color='white'>Password is successfully changed.</font></h4>";
					} else {
						echo "<h4 style='width:530px;height:30px;background:red'><font color='white'>Error updating password: " . $stmt->error . "</font></h4>";
					}
				} else {
					echo "<h4 style='width:530px;height:40px;background:red' align='left'><font color='white'><font color='green' size='5'>(?)</font> Please enter correct old password.</font></h4>";
				}
