...
";
	...
	<th width="1047" height="436" valign="top" bgcolor="white"><br>
		...
		$date1 = date('Y-m-d');
		$result = mysqli_query($conn,"SELECT * FROM marrage_idcard where id='$id1' ");
		...
		$mysqlquery = "INSERT INTO `population`(`id`,`HouseNumber`, `FirstName`, `middleName`, `LastName`, `age`, `Sex`, `Kebele`, `Job`, `PlaceOfBirth`, `DateOfBirth`, `DateOfRegistration`, `Nationality`, `UploadPicture`) VALUES ('$id','" . $house . "','" . $fname1 . "','" . $mname1 . "','" . $lname . "','" . $age . "','" . $sex . "','" . $kebele . "','" . $job . "','" . $address . "','" . $birthdate . "','" . $date1 . "','" . $nation . "','" . $profile . "')";
		$sql = mysqli_query($conn,$mysqlquery);
		if ($sql) {

			$up = "INSERT INTO `registernoti`(`id`, `sendto`, `from`, `content`, `HouseNumber`, `date`,`seen`) VALUES ('" . $id . "','admin','$_username','please creat account for $fname1','$house','" . $date1 . "','0')";
			$ge = mysqli_query($conn,$up);
			$sql2 = "INSERT INTO `newnoti`(`id`, `Sender_name`, `purpose`, `date`, `send`) VALUES('$id','$_username','The requeste for register is Accepted.using first name.middle name you can login and password is kebele','$date1','$fname')";
			$res = mysqli_query($conn,$sql2);
			$s = "UPDATE `marrage_idcard` SET `status`=1 WHERE id='$id'";
			$r = mysqli_query($conn,$s);

			echo "<h4 style='width:550px;height:40px; background:brown;'><font color='white'>You are succesfully accept " . $fname1 . "." . $mname1 . "  registration request.</font></h4>";
		} else {
			$s = "UPDATE `marrage_idcard` SET `status`=1 WHERE id='$id'";
			$r = mysqli_query($conn,$s);
			echo "<h4 style='width:550px;height:40px; background:brown;' align='left'><font color='white'><font color='grean' size='6'>(!) </font> You accepted the request before</font></h4>";
		}
		mysqli_close($conn);
		?>
