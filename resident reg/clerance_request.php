...
";
	...
	<th width="1047" height="300" valign="top" bgcolor="white"><br>
		...
		$result = mysqli_query($conn,"SELECT *FROM idcard where FirstName='$fname' ");
		$con = mysqli_num_rows($result);
		while ($row = mysqli_fetch_array($result)) {
			$id = $row['id_number'];
		}
		if ($con < 1)
			echo "<h4 style='width:540px;height:40px;background:brown;' align='left'><font color='white'><font color='grean' size='6'>(!)</font> You are not allowed this page.please send idcard requset first.</font></h4>";

		else {
		?>

			<div><b> Welcome to clearance request form</b></div>
			<hr />
			<fieldset>
				<legend align="center">
				...
				<div>
					<form name="request" onsubmit='return formValidation()' action="clerance_request.php" method='POST'>
						...
						<table align="center">
							<tr>
								<td>Id Number :</td>
								<td><input type="text" size="20px" id='house' name='Id' value="<?php echo $id; ?>" readonly></td>
							</tr>

							<td><label for="file">Police service certifcate:</label></td>
							...
							<tr>
								...
								<td><input type="submit" name="submit" style="cursor:pointer" value="Send request" onClick="return check(this.form);" />
									<input type='reset' value='clear'><a href="resident.php">
								</td>
							</tr>
						</table>
					</form>
			...
			if (isset($_POST['submit'])) {
				...
				$count = 1;
				$sql = "SELECT * FROM `clearance_requset`";
				...
				aa:
				if ($count == 0) {
					$vara = "UPDATE `clearance_requset` SET `id_number`='$id',`send`='$_username',`reciver`='$rec',`polce_cerifcate`='" . $photo . "',`smll_cirtifcate`='" . $phot . "',`date`='$date1',`seen`=0 WHERE id_number='$id'";
					if (mysqli_query($conn,$vara))
						echo "<h4 align='left' style='width:500px;height:30px;background:brown;'><font color='white'><font color='grean' size='5'>(=)</font>Request again  successfully sent'.</font></h4>";
				} else if ($count == 1) {
					$sql = "INSERT INTO `clearance_requset`(`id_number`, `send`, `reciver`, `polce_cerifcate`, `smll_cirtifcate`, `content`, `date`, `seen`) VALUES
('$id','$_username','$rec','" . $photo . "','" . $phot . "','" . $pur . "','$date1','0')";
					if (!mysqli_query($conn,$sql)) {
						die('Error: ' . mysqli_error($conn));
					} else {
						echo "<h4 align='left' style='width:500px;height:30px;background:brown;'><font color='white'><font color='grean' size='5'>(=)</font>Request for clerance successfully sent'.</font></h4>";
					}
				}
			}
		}
