...
";
	...
	<th width="1047" height="436" valign="top" bgcolor="white"><br>
		<?php
		$id = $_REQUEST['key1'];
		$result1 = mysqli_query($conn,"SELECT * FROM marrage_idcard  where id='{$id}'");
		while ($rn = mysqli_fetch_array($result1)) {
			$name1 = $row['sender'];
			# code...
		}
		$result2 = mysqli_query($conn,"SELECT * FROM user  where Username='$name1'");
		while ($rn = mysqli_fetch_array($result2)) {
			$finame = $row['frist_name'];
			$mname = $row['mname'];
			$fu = $fname . " " . $mname;
			# code...
		}

		$result = mysqli_query($conn,"SELECT * FROM marrage_idcard  where id ='$id'");
		echo "<table id='vtable' style='width:530px;border:px solid #eee;' align='lef' valign='top'><font color=white>
<tr>
<th bgcolor='#eee'><font color='blue' size='2'>  sender name </th>
<th bgcolor='#eee'><font color='blue' size='2'>Purpose </th>
<th bgcolor='#eee'><font color='blue' size='2'>cirtifcate </th>
<th bgcolor='#eee'><font color='blue' size='2'>Aprrove </th>
<th bgcolor='#eee'><font color='blue' size='2'>Reject </th>
</tr>";
		...
		while ($row = mysqli_fetch_array($result)) {
			...
			echo $row["content"];
			echo "</td>";
			echo "<td align=center><a title='Click here to download in file.' href='downlod.php?id={$row['cirtficate']}'>Dawonlod </a>";
			echo ("<td align = 'center' width = '1'><a href = 'approve.php?id=" . $list . "' onClick=\"return confirm('Are you shure approve the $ctrl requset.')\"><img src = 'image/app.png' border = '0' title='approve registration' width='40';'></img></a>
    </td>");
			echo ("<td align = 'center' width = '1'><a href = 'disapprove.php?content=" . $ctrl . "&id=" . $list . "' onClick=\"return confirm('Are you shure disapprove $ctrl request.')\"><img src = 'image/remove.png' width='30' border = '0' title='reject registratio';'></img></a>
    </td>");
			echo ("</tr>");
		}
		echo ("</table>");
		...
		<a href="approve_register.php"><img src="image/b.png" width="40"> </a>
