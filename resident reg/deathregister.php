...
";
	...
	<th width="1047" height="436" valign="top" bgcolor="white"><br>
		...
		<div><b>Search Resident information by house number to register death!!</b></div>
		<hr />
		<form action="deathregister.php" onsubmit='return formValidator()' method='POST'>
			<table align="center">
				<tr>
					<td class="search"> House Number:</td>
					<?php
		...
		if (isset($_POST['search'])) {
			...
			$idno = $_POST['searchs'];
			$sql = "SELECT * FROM population WHERE HouseNumber='" . $idno . "'";
			$result = mysqli_query($conn,$sql);
			$count = mysqli_num_rows($result);
			if ($count < 1) {
				echo "This house number is not found";
			} else {
				...
				while ($r = mysqli_fetch_array($result1)) {
					...
					if ($st == 0) {
						echo "<center>";
						print("<tr>");
						print("<td>" . $r['HouseNumber'] . "</td>");
						print("<td>" . $r['FirstName'] . '&nbsp;' . $r['middleName'] . '&nbsp;' . $r['LastName'] . "</td>");
						print("<td align = 'center' width = '1'><a href = 'death.php?id=" . $r['id'] . "'><img src = 'image/Edit.png' width='25px' height='25px' title='Prepare id card' ></a></td>");
						print("</tr>");
					} else if ($st == 1)
						continue;
				}
