...
";
	...
	<th width="1047" height="300" valign="top" bgcolor="white"><br>
		...
		<form action='uploadpho.php' method='POST' enctype="multipart/form-data">
			<table valign='top' align="center" style="width:400px; margin-top:-110px; margin-top:-220px; height:150px;border-radius:16px;border:2px solid #686868">
				<tr>
					<td colspan="2" align="right"> <a href="resnoti.php"><img src="image/close.png" width="30" title="Click here to back serach menu"></a></td>
				</tr>
				<tr>
					<td>Id number:</td>
					<td><input type='number' name='id' placeholder="Enter your id number"> </td>
				</tr><br />
				<tr>
					<td>UploadPicture:</td>
		...
		if (isset($_POST['update'])) {
			...
			$updat = mysqli_query($conn,"update clearance set profile='" . $photo . "' where IdNumber='$id1'") or die(mysqli_error($conn));
			if ($updat) {
				$sql = "SELECT * FROM `clearance` WHERE IdNumber='$id1'";
				$res = mysqli_query($conn,$sql);
				while ($row = mysqli_fetch_array($res)) {
					$pro = $row['profile'];
					# code...
