...
";
	...
	<th width="1047" height="436" valign="top" bgcolor="white"><br>
		...
		$num = $_REQUEST['House'];
		$id1 = $_REQUEST['name'];
		$sql = "UPDATE `request` SET `seen`='1' WHERE id='$id1'";
		$upd = mysqli_query($conn,$sql);

		?>
		<div><b>Search resident information by house number!!</b></div>
		<hr />
		<form action="display_residnet.php" onsubmit='return formValidator()' method='POST'>
			<table align="center">
				<tr>
					<td class="search">ID Number:</td>
					<td><input type="text" name="searchs" size="40px" value="<?php echo $num; ?>" placeholder="Provide Here..." style="width: 200;height: 30; background:#eee;" /></td>
		...
		if (isset($_POST['search'])) {
			...
			$idno = $_POST['searchs'];
			$sql = "SELECT * FROM population WHERE id='" . $idno . "'";
			$result = mysqli_query($conn,$sql);
			$count = mysqli_num_rows($result);
			if ($count < 1) {
				echo "This id number is not found";
			} else {
				...
				while ($r = mysqli_fetch_array($result1)) {
					...
					if ($st == 0) {
						...
						print("<tr>");
						print("<td>" . $r['id'] . "</td>");
						print("<td>" . $r['FirstName'] . '&nbsp;' . $r['middleName'] . '&nbsp;' . $r['LastName'] . "</td>");
						print("<td align = 'center' width = '1'><a href = 'prepare_idcard.php?id=" . $r['id'] . "'><img src = 'image/Edit.png' width='25px' height='25px' title='Prepare id card' ></a></td>");
						print("</tr>");
					} else if ($st == 1)
						continue;
				}
