...
";
	...
	<th width="1047" height="436" valign="top" bgcolor="white"><br>
		<table>
			<tr>
				<td>
					...
					<h3>
						<?php
						echo "Renew ID Card of residents";
						$id = $_REQUEST['id'];
						$name = $_REQUEST['name'];
					...
					<form method="post">
						<br /><b style="margin-left:40px;"> <?php echo "Enter a id number to search";  ?> : <br />
							<span style="width:140px;">
								<input style="float:left;" class="form-input" type="text" name="searchtxt" required="" value="<?php echo $id; ?>" />
		...
		if (isset($_POST['btnsearch'])) {
			...
			$result = mysqli_query($conn,"SELECT  * FROM idcard  WHERE id_number='$key'");
			if ($result) {
				echo '<div style="overflow:auto; width:100%; height:300px; ">';
				echo '<table border=2  class="table table-striped table-hover table-condensed table-bordered" style=" background-color:white;">';
				echo '<tr>';
				echo '<th>ID Number</th>';
				echo '<th>Full Name </th>';
				echo '<th> Job</th>';
				echo '<th> Nationality</th>';
				echo '<th> Renewed Date </th>';
				echo '<th >Operation</th>';
				echo '</tr>';
				while ($run = mysqli_fetch_array($result)) {
					$id_number = $run['id_number'];
					$fname = $run['FirstName'];
					$mname = $run['middleName'];
					$full_name = $fname . " " . $mname . " ";
					$job = $run['Job'];
					$nation = $run['Nationality'];
					$rn_date = $run['renewed_date'];
					echo '<tr>';
					...
					echo '<td>' . $nation . '</td>';
					echo '<td>' . $rn_date . '</td>';
					echo '<td><a href="renew1.php?s_id=' . $id_number . '&name=' . $name . '">Renew</a></td>';
					echo '</tr>';
				}
				echo '</table>';
				echo '</div>';
