...
";
	...
	<th width="1047" height="436" valign="top" bgcolor="white"><br>
		...
		<div><b>
				<center>House Registration Form</center>
			</b></div>
		<hr />
		<fieldset>
			<legend align="center">
			...
			<br>
			<div>
				<form onsubmit='return formValidation()' action='house.php' method='POST'>
					<table cellspacing="15px">
						<tr>
							<td>
								<font color="red"></font>Owner Id :
							</td>
							<?php
							echo "<td>" . "<select id='job' name='id' style=\"width: 200;height: 30; background:#eee;\">";
							echo "<option>Please Choose id</option>";

							$sql = "SELECT * FROM `idcard`";
							$res = mysqli_query($conn, $sql);
							while ($row = mysqli_fetch_array($res)) {
								# code...
								$house_num = $row['id_number'];
						...
						<tr>
							<td>
								<font color="red"></font>House Type :
							</td>
							<td><input type="radio" name="housetype" id="optionsRadios1" value="Kebele House" checked>
								Kebele House<input type="radio" name="housetype" id="optionsRadios2" value="Private house" checked>
								Private house
							</td>
						...
						<tr>
							<td>
								<font color="red"></font>House Number :
							</td>
							<?php
							$sql = "SELECT * FROM `registernoti`";
							$res = mysqli_query($conn, $sql);
							...
							?>
							<td><input type="text" size="20px" id='house' name='house' value="<?php echo $house + 1; ?>" readonly style="width: 200;height: 30; background:#eee;"></td>
						</tr>
						<tr>
							<td>
								<font color="red"></font>Square meter:
							</td>
							<td><input type="text" size="20px" id='region' name='region' placeholder="Enter squre meter... " style="width: 200;height: 30; background:#eee;" pattern="[0-9- ]+" required title="Enter only alphabet"></td>
						</tr>
		...
		if (isset($_POST['houses'])) {
			...
			$oid = mysqli_real_escape_string($conn, $_POST['id']);
			...
			$sql = "INSERT INTO house (OwnerId, HouseNumber, Kebele, Region, House_Type, DateOfRegistration)
            VALUES (?, ?, ?, ?, ?, ?)";

			$stmt = mysqli_prepare($conn, $sql);
			mysqli_stmt_bind_param($stmt, "ssssss", $oid, $house, $kebele, $region, $housetype, $da);

			// Execute the prepared statement
			if (mysqli_stmt_execute($stmt)) {
				echo "<h4 style='width:520px;height:30px;background:brown'><font color='white'> House Registered Successfully </font></h4>";
			} else {
				// Handle errors
