...
";
	...
	<th width="1047" height="436" valign="top" bgcolor="white"><br>
		...
		if ($no > 0) {
			echo "<font color='white'><h4 style='width:530px;height:30px;background:brown;'>Welcome " . htmlspecialchars($ful) . " you have <font color=''>" . $no . "</font> new comments.</h4></font>";
			echo "<table border=0 >";

			$i = 1;
			while ($row = mysqli_fetch_assoc($result)) {
				$ctr2 = $row['Comment'];
				echo "<tr><td colspan='4'><h3 align='center'><font color='blue'>Comment(" . $i . ")</font></h3></td></tr>";
				echo "<tr bgcolor='#eee'><th>From</th><th>Date</th><th>Comment</th><th colspan=3 width=10%>Action</th></tr>";
				echo "<tr><td width=5% align='center'>" . htmlspecialchars($row['from']) . "</td><td width=15%>" . htmlspecialchars($row['date']) . "</td><td width=15%>" . htmlspecialchars($row['Comment']) . "</td>";
				echo "<td width=4% align='center'><a href='deletecomment_charman.php?key2=" . urlencode($ctr2) . "' onClick=\"return confirm('Are you sure to delete the comment?')\"><img src='image/Delete.png' alt='delete'></a></td></tr>";
				$i++;
			}
			echo "</table>";
		} else {
			echo "<h4 align='left' style='width:530px;height:30px;background:red;'><font color='white'><font color='green' size='5'>(!)</font> No new comments from users.</font></h4>";
		}
