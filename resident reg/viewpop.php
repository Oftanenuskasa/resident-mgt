...
";
	...
	<th width="1047" height="436" valign="top" bgcolor="white"><br>
		...
		<table>
			<tr>
				<td>
					<div><b> Well came &nbsp;<font color='blue'><?php echo $ful; ?></font>&nbsp;please enter house number to view resident information</b></div>
					<hr />
					<form action="viewpop.php" onsubmit='return formValidator()' method='POST'>
						<table>
							<tr>
								<td class="search">Enter ID_Number:</td><?php
																		echo "<td>" . "<select id='job' name='searchs' style=\"width: 200;height: 30; background:#eee;\">";
																		echo "<option>Please Choose id number</option>";
																		...
																		echo "</td>" . "</select>";
																		?>
								<td><input type="submit" value="Search" name="search" style="cursor:pointer;" />
								</td>
							</tr>
						</table>
					</form>
					...
					if (isset($_POST['search'])) {
						$house = $_POST['searchs'];
						$sql = "SELECT * FROM population WHERE id='{$house}'";
...
<th>Edit</th>
							...
							while ($row = mysqli_fetch_array($result)) {
								...
								print("<td align = 'center' width = '1'><a href = 'viewdetail.php?key1=" . $id . "'><img width='25px' height='25px' src = 'image/search.png' title='View Detail'></a></td>
         <td align = 'center' width = '1'><a href = 'edit_population.php?key1=" . $id . "' onClick=\"return confirm('Are you shure to  Edit id $id Data.')\"><img src = 'image/Edit.png' width='25px' height='25px' title='Edit' ></a></td>
		");
								print("</tr>");
							}
							print("</table>");
							...
							echo "<font color='red'>No resident lived in this house.</font>";
					}
					mysqli_close($conn);
					?>
				</td>
