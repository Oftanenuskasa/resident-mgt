...
";
	...
	<th width="1047" height="436" valign="top" bgcolor="white"><br>
		...
		$mysqlquery = "INSERT INTO `population`(`id`, `HouseNumber`, `FirstName`, `middleName`, `LastName`, `age`, `Sex`, `Kebele`, `Job`, `PlaceOfBirth`, `DateOfBirth`, `DateOfRegistration`, `Nationality`, `UploadPicture`) VALUES ('" . $id . "','" . $house . "','" . $fname . "','" . $mname . "','" . $lname . "','" . $age . "','" . $sex . "','" . $kebele . "','" . $job . "','" . $address . "','$birthdate','" . $date1 . "','" . $nation . "','" . $profile . "')";
		...
		if ($sql) {
			...
			$sql1 = "INSERT INTO `registernoti`(`id`, `sendto`, `from`, `content`, `HouseNumber`, `date`,`seen`) VALUES ('" . $id . "','admin','$_username','please creat account for $fname','" . $house . "','$date1','0')";
			$ress = mysqli_query($conn,$sql1);
			$sql = "INSERT INTO `request`(`id`, `name`, `SenderHouseNumber`, `content`, `date`, `seen`) VALUES ('none','$_username','" . $house . "','Please prepare Id card for $fname','$date1','0')";
			$re = mysqli_query($conn,$sql);
			if ($re) {
				$s = "UPDATE `marrage_idcard` SET `status`=1 WHERE id='$id'";
				$r = mysqli_query($conn,$s);

				echo "<h4 style='width:530px;height:40px;background:brown;'><font color='white'>The requset is succesfully Accepted.</font></h4>";
			}
		} else {
			$s = "UPDATE `marrage_idcard` SET `status`=1 WHERE id='$id'";
			$r = mysqli_query($conn,$s);

			echo "<h4 style='width:530px;height:40px;background:brown;'><font color='white'>This requset accepted before.</font> </h4>";
		}
		mysqli_close($conn);
		?>
		<a href="approve_register.php"><img src="image/b.png" title="Click here to back to approve_register page.For serch agin."></a>
