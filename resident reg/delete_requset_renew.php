...
";
	...
	<th width="1047" height="436" valign="top" bgcolor="white"><br>
		...
		$dat1 = date('Y-m-d');
		$id = $_REQUEST['content'];
		$sql = "SELECT * FROM `requestr` WHERE id='$id'";
		$res = mysqli_query($conn,$sql);
		while ($ro = mysqli_fetch_array($res)) {
			$nam = $ro['sendername'];	# code...
		}
		$sql = "INSERT INTO `noti`(`id`, `from`, `content`, `send`, `seen`, `date`, `username`) VALUES ('none','$_username','The request for id prepare is not accepted.','$nam','0','$dat1','none')";
		if (mysqli_query($conn,$sql)) {
			mysqli_query($conn,"DELETE FROM requestr WHERE id= '$id'")
				or die(mysqli_error($conn));

			echo "<h4 align='center' style='width:530px;height:30px;background:green;><font color='red'> The data requset reject successfully. !!</h4></font>";
		}
