...
";
	...
	<th width="1047" height="436" valign="top" bgcolor="white"><br>
		...
		$result = mysqli_query($conn, "SELECT *FROM newnoti where FirstName='$fname' ");
		...
		if ($con > 0) {
		?>
			<div><b> Well come to renew or missing Idcard request form</b></div>
			...
			if (isset($_POST['submit'])) {
				...
				if ($da == $renw and $content = 'renew') {
					echo "<font color='red'><h4 align='center'> ID is allredy renwed in Today!!</h4></font";
				} else if ($da >= $renw and $da < $exp and $content = 'renew')
					echo "<font color='red'><h4 align='center'> In this year is not allowed to renew!!</h4></font";
				else {
					$sql = "SELECT  * FROM `user` WHERE Username='$_username'";
					$re = mysqli_query($conn, $sql);
					if (!$re)
...
('none','$_username','$_POST[first]','$da', '$_POST[com]','0')";
					...
					if (!$result) {
						...
						echo "<font color='red'><h4 align='center'> Your request successfully sent!!</h4></font";
				}
			}
		} else
			echo "<h4 style='width:540px;height:50px;background:brown;'><font color='white'>You are not allowed to acces this page.To acces this page you must have Id card first.</font></h4>";
