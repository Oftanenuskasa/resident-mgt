...
";
	...
	<th width="1047" height="436" valign="top" bgcolor="white"><br>
		<table>
			<tr>
				<td>
					<div><b>View new Form</b></div>
					<hr />
					<form onsubmit='return formValidator()' action="viewnew.php" method='POST'>
						<table>
							<tr>
								<td class="search">Enter NewsId:</td>
								<?php
								echo "<td>" . "<select id='job' name='searchs' style=\"width: 220;height: 30; background:#eee;\">";
								echo "<option>Please Choose news identifcation </option>";

								$sql = "SELECT * FROM `news`";
								$res = mysqli_query($conn,$sql);
								while ($row = mysqli_fetch_array($res)) {
									# code...
									$house_num = $row['NewsId'];
					...
					if (isset($_POST['search'])) {
						$idno = $_POST['searchs'];
						$sql = "SELECT * FROM news WHERE NewsId='{$idno}'";
						$result = mysqli_query($conn,$sql);
						$count = mysqli_num_rows($result);
						if ($count > 0) {
							echo "<center>";
							echo "<table border='1' style='width:530px;border-radius:10px;' align='center'>
<tr>
<th width='150px'>NewsId.</th>
...
<th>Delete</th>
							...
							while ($row = mysqli_fetch_array($result)) {
								$ctrl = $row['NewsId'];
								print("<tr>");
								print("<td>" . $row['NewsId'] . "</td>");
								print(" <td align = 'center' width = '1'><a href = 'edit_new.php?key1=" . $ctrl . "'><img src = 'image/Edit.png' width='25px' height='25px' title='Edit' ></a></td>
		<td align = 'center' width = '1'><a href = 'deletenew.php?key1=" . $ctrl . "'onClick=\"return confirm('Are you shure to delete.')\"><img width='25px' height='25px' src = 'image/Delete.png' title='Delete'></img></a></td>
		");
								print("</tr>");
							}
							print("</table>");
							...
							echo "<font color='red'>This NewsId is not found,</font>";
					}
					mysqli_close($conn);
					?>
				</td>
