...
";
		...
		<table id="contentbox" border="1" width="100%">
			<tr>
				<td>
					...
					<div id="print_content">
						<div align="center" style="margin-top:10px;"><strong><img src="image/repo1.jpg" width="100%" height="80" align="center"></strong></div>
						<div style="margin-left:130%; width:50px;  text-align:center;"></div>
						<br>
						<?php
						$result1 = mysqli_query($conn, "SELECT * FROM clearance WHERE Sex='Female' AND job='Farmer'");
						$fmale_far = mysqli_num_rows($result1);
						
						$result2 = mysqli_query($conn, "SELECT * FROM clearance WHERE Sex='Male' AND job='Farmer'");
						$male_far = mysqli_num_rows($result2);
						$total_far = $fmale_far + $male_far;
						
						$result3 = mysqli_query($conn, "SELECT * FROM clearance WHERE Sex='Female' AND job='Student'");
						$fmale_stu = mysqli_num_rows($result3);
						
						$result4 = mysqli_query($conn, "SELECT * FROM clearance WHERE Sex='Male' AND job='Student'");
						$male_stu = mysqli_num_rows($result4);
						$total_stu = $fmale_stu + $male_stu;
						
						$result5 = mysqli_query($conn, "SELECT * FROM clearance WHERE Sex='Female' AND job='Teacher'");
						$fmale_th = mysqli_num_rows($result5);
						
						$result6 = mysqli_query($conn, "SELECT * FROM clearance WHERE Sex='Male' AND job='Teacher'");
						$male_th = mysqli_num_rows($result6);
						$total_th = $fmale_th + $male_th;
						
						$result7 = mysqli_query($conn, "SELECT * FROM clearance WHERE Sex='Female' AND job='Doctor'");
						$fmale_doc = mysqli_num_rows($result7);
						
						$result8 = mysqli_query($conn, "SELECT * FROM clearance WHERE Sex='Male' AND job='Doctor'");
						$male_doc = mysqli_num_rows($result8);
						$total_doc = $fmale_doc + $male_doc;
						
						$result9 = mysqli_query($conn, "SELECT * FROM clearance WHERE Sex='Female' AND job='Guard'");
						$fmale_ga = mysqli_num_rows($result9);
						
						$result10 = mysqli_query($conn, "SELECT * FROM clearance WHERE Sex='Male' AND job='Guard'");
						$male_ga = mysqli_num_rows($result10);
						$total_ga = $fmale_ga + $male_ga;
						
						$result11 = mysqli_query($conn, "SELECT * FROM clearance WHERE Sex='Female' AND job='Phd'");
						$fmale_ph = mysqli_num_rows($result11);
						
						$result12 = mysqli_query($conn, "SELECT * FROM clearance WHERE Sex='Male' AND job='Phd'");
						$male_ph = mysqli_num_rows($result12);
						$total_ph = $fmale_ph + $male_ph;
						
						$result13 = mysqli_query($conn, "SELECT * FROM clearance WHERE Sex='Female' AND job='Master'");
						$fmale_ms = mysqli_num_rows($result13);
						
						$result14 = mysqli_query($conn, "SELECT * FROM clearance WHERE Sex='Male' AND job='Master'");
						$male_ms = mysqli_num_rows($result14);
						$total_ms = $fmale_ms + $male_ms;
						
						$result15 = mysqli_query($conn, "SELECT * FROM clearance WHERE Sex='Female' AND job='Other'");
						$fmale_oth = mysqli_num_rows($result15);
						
						$result16 = mysqli_query($conn, "SELECT * FROM clearance WHERE Sex='Male' AND job='Other'");
						...
						$result_all = mysqli_query($conn, "SELECT * FROM clearance");
			...
			<tr>
				<td colspan="7" align="center" bgcolor="#eee">
					<font color='blue'><b>General clearance report that is prepared.</b></font>
				</td>
			</tr>
			...
			<tr>
				<td colspan="4">The total number of Clearance prepared:</td>
	...
	<?php
							...
							echo "<font color='black'>No clerance prepared beteewn this date.</font>";
						}
	?>
	...
	<div style="margin-left:70%; width:50px;  text-align:center;"> <a href="javascript:Clickheretoprint()"><img align="center" src="image/DPIC_36483.jpg" title="Click here to print the report" /></a>
	</div>
