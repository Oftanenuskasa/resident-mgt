...
";
	...
	<th width="1047" height="436" valign="top" bgcolor="white"><br>
		...
		<table>
			<tr>
				<td>
					<div><b> Welcome &nbsp;<font color='blue'><?php echo $ful; ?></font>&nbsp;please enter house number to view house information</b></div>
					<hr />
					<form onsubmit='return formValidator()' action="viewhouse.php" method='POST'>
						<table>
							<tr>
								<td class="search">Enter House Number:</td><?php
																			echo "<td>" . "<select id='job' name='searchs' style=\"width: 230;height: 30; background:#eee;\">";
																			...
																			echo "</td>" . "</select>";
																			?>
								<td><input type="submit" value="Search" name="search" style="cursor:pointer; background:green; height: 30px; " />
								</td>
							</tr>
						</table>
					</form>
					...
					if (isset($_POST['search'])) {
						$idno = $_POST['searchs'];
						$sql = "SELECT * FROM house WHERE HouseNumber='{$idno}'";
						$result = mysqli_query($conn,$sql);
						$count = mysqli_num_rows($result);
						if ($count < 1) {
							echo ('<font color="red">This House Number is not found</font>');
						} else {
							echo "<center>";
							echo "<table border='1' style='width:500px;border-radius:10px;' align='center'>
<tr>
...
<th>Delete</th>
							...
							while ($row = mysqli_fetch_array($result)) {
								...
								print("<td align = 'center' width = '1'><a href = 'viewdetails.php?key1=" . $ctrl . "'><img width='25px' height='25px' src = 'image/search.png' title='View Detail'></a></td>
         <td align = 'center' width = '1'><a href = 'edithouse.php?key1=" . $ctrl . "' onClick=\"return confirm('Are you shure to edit the $ctrl Data.')\"><img src = 'image/Edit.png' width='25px' height='25px' title='Edit' ></a></td>
		<td align = 'center' width = '1'><a href = 'deletehouse.php?key1=" . $ctrl . "' onClick=\"return confirm('Are you shure to delete $ctrl Data.')\"><img width='25px' height='25px' src = 'image/Delete.png' title='Delete'></img></a></td>
		");
								print("</tr>");
							}
							print("</table>");
